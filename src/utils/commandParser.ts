...
export function parseSearchArgs(args: string[]): {
  ...
  const maybeEngine = findEngineByName(sub);
  if (maybeEngine) {
    return {
      action: 'search',
...
export function parseToArgs(args: string[]): { action: string; name?: string; url?: string } {
  if (args.length === 0) {
...
export function parseAIArgs(args: string[]): { action: string; key?: string; value?: string; query: string } {
  if (args.length === 0) {
...
export function parseConfigArgs(args: string[]): { module: string; action: string; key?: string; value?: string } {
  if (args.length >= 2) {
    return {
      module: args[0],
      action: args[1],
