...
import { parseCommand, parseSearchArgs, parseToArgs, parseAIArgs, parseConfigArgs } from '@/utils/commandParser';
...
const handleSearch = (args: string[]) => {
  const { action, query, engineName, engineUrl } = parseSearchArgs(args);
  switch (action) {
    case 'search': {
      if (engineName) {
    ...
    case 'add': {
      ...
      } catch (e) { terminalStore.addOutput(`✗ ${(e as Error).message}`, 'error'); }
      break;
    }
    case 'list': {
      const engines = searchStore.engines;
      if (!engines.length) { terminalStore.addOutput('暂无搜索引擎', 'warning'); return; }
      terminalStore.addOutput(
        engines.map(e => `  ${e.name.padEnd(14)} ${e.id === searchStore.defaultEngine ? '★ ' : '  '}${e.url}`).join('\n'),
        'success'
      );
      break;
    }
    case 'default': {
      if (!engineName) { terminalStore.addOutput('用法: search default <引擎名>', 'warning'); return; }
      const engine = searchStore.engines.find(
        e => e.name.toLowerCase() === engineName.toLowerCase() || e.id === engineName.toLowerCase()
      );
      if (!engine) { terminalStore.addOutput(`✗ 搜索引擎不存在: ${engineName}`, 'error'); return; }
      searchStore.setDefaultEngine(engine.id);
      terminalStore.addOutput(`✓ 默认搜索引擎已设置为: ${engine.name}`, 'success');
      break;
    }
    case 'delete': {
      if (!engineName) { terminalStore.addOutput('用法: search delete <引擎名>', 'warning'); return; }
      const engine = searchStore.engines.find(
        e => e.name.toLowerCase() === engineName.toLowerCase() || e.id === engineName.toLowerCase()
      );
      if (!engine) { terminalStore.addOutput(`✗ 搜索引擎不存在: ${engineName}`, 'error'); return; }
...
const handleTo = (args: string[]) => {
  ...
  switch (action) {
    case 'list': {
      if (!shortcutsStore.shortcuts.length) { terminalStore.addOutput('暂无快捷指令', 'warning'); return; }
      terminalStore.addOutput(shortcutsStore.shortcuts.map(s => `  ${s.name.padEnd(16)} →  ${s.url}`).join('\n'), 'success');
      break;
    }
    case 'add': {
      if (!name || !url) { terminalStore.addOutput('用法: to add <名称> <URL>', 'warning'); return; }
      try { shortcutsStore.addShortcut(name, url); terminalStore.addOutput(`✓ 已添加: ${name}`, 'success'); }
      catch (e) { terminalStore.addOutput(`✗ ${(e as Error).message}`, 'error'); }
      break;
    }
    case 'edit': {
      if (!name || !url) { terminalStore.addOutput('用法: to edit <名称> <URL>', 'warning'); return; }
      try {
        const sc = shortcutsStore.findShortcutByName(name);
        if (!sc) { terminalStore.addOutput(`✗ 指令不存在: ${name}`, 'warning'); return; }
    ...
    case 'delete': {
      if (!name) { terminalStore.addOutput('用法: to delete <名称>', 'warning'); return; }
      try {
        const sc = shortcutsStore.findShortcutByName(name);
        if (!sc) { terminalStore.addOutput(`✗ 指令不存在: ${name}`, 'warning'); return; }
    ...
    case 'execute': {
      if (!name) { terminalStore.addOutput('用法: to <指令名>', 'warning'); return; }
...
const handleAI = (args: string[]) => {
  ...
  switch (action) {
    case 'config': {
      if (!key) { terminalStore.addOutput('用法: ai config set <key> <value>', 'warning'); return; }
      try {
        aiStore.updateConfig(key as keyof typeof aiStore.config, value);
        terminalStore.addOutput(`✓ AI 配置已更新: ${key} = ${key === 'apiKey' ? '••••••' : value}`, 'success');
      } catch (e) { terminalStore.addOutput(`✗ ${(e as Error).message}`, 'error'); }
      break;
    }
    case 'clear': { aiStore.clearMessages(); terminalStore.addOutput('✓ AI 对话历史已清空', 'success'); break; }
    case 'chat': {
      if (!query) { terminalStore.addOutput('请输入问题', 'warning'); return; }
      if (!aiStore.hasApiKey) { terminalStore.addOutput('⚠ 请先配置 API 密钥:\n  ai config set apiKey <your_key>', 'warning'); return; }
      terminalStore.addOutput('🤖 思考中...', 'info');
      aiStore.sendMessageToAI(query)
...
const handleConfig = (args: string[]) => {
  ...
  if (module === 'search' && action === 'set') {
    if (!key) { terminalStore.addOutput('用法: config search set <key> <value>', 'warning'); return; }
    if (key === 'defaultEngine') {
      if (!value) { terminalStore.addOutput('请输入搜索引擎 ID', 'warning'); return; }
      searchStore.setDefaultEngine(value);
      terminalStore.addOutput(`✓ 默认搜索引擎: ${value}`, 'success');
    } else { terminalStore.addOutput(`✗ 未知配置项: ${key}`, 'error'); }
  } else { terminalStore.addOutput('✗ 用法: config search set <key> <value>', 'warning'); }
};
...
const executeCommand = () => {
  ...
  switch (parsed.type) {
    ...
    case 'history': {
      ...
      else terminalStore.addOutput(list.map(h => `  ${String(h.index).padStart(3)}  ${h.command}`).join('\n'), 'info');
      break;
    }
    case 'search': handleSearch(parsed.args); break;
    case 'ai': handleAI(parsed.args); break;
    case 'to': handleTo(parsed.args); break;
    case 'config': handleConfig(parsed.args); break;
  }
  scrollToBottom();
};
...
const showWelcome = () => {
  ...
  terminalStore.addOutput('', 'output');
  terminalStore.addOutput('  CLI Home — Terminal Browser Home  v1.0.0', 'success');
  terminalStore.addOutput('  输入 help 查看可用命令', 'output');
  terminalStore.addOutput('', 'output');
};

onMounted(() => {
  terminalStore.loadCommandHistoryFromStorage();
  if (terminalStore.history.length === 0) { showWelcome(); scrollToBottom(); }
});
</script>

<template>
  <div class="h-full w-full flex flex-col" style="background: #0c0c12;">
    <div ref="outputRef" class="flex-1 overflow-y-scroll overflow-x-hidden px-5 py-4 font-mono text-[13px] leading-relaxed space-y-0.5">
